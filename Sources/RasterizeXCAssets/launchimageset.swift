...
public func launchImageSet(source: String, outputURL: URL) -> Promise<Void> {
    let fs = FileManager.default
    let worker = Rasterizer()

    let sizes: [(UInt,UInt)] = [(1024,768), (2048,1536), (768,1024), (1536,2048)]
    let images = sizes.map { (width,height) in worker.rasterize(source: source, width: width, height: height).map { ((width, height), $0) } }
    ...
    return firstly {
        ...
        for (size, image) in images {
          let (width, height) = size
            try image.write(to: outputURL.appendingPathComponent("LaunchImage-\(width)-\(height).png"))
        }
